...
class Privatezone():
    def __init__(self, Baseurl="https://pvtz.aliyuncs.com/?", Format="xml", Version="2018-01-01",
                 ...
                 SignatureNonce=uuid1(), Action="", ZoneId=""):
...
def main(ZoneId="", AccessKeyId=""):
    hostname = gethostname()
    ipaddr = getip()
    Rt = hostname + '_' + ".".join(ipaddr.split(".")[2:4])
    DNS = Privatezone(Format="json", Action="DescribeZoneRecords", ZoneId=ZoneId, AccessKeyId=AccessKeyId)
    recordId = DNS.getrecord(IP=ipaddr)
    if recordId == '':
        try:
            DNS = Privatezone(Action="AddZoneRecord", ZoneId=ZoneId, AccessKeyId=AccessKeyId)
        ...
        try:
            DNS = Privatezone(Action="UpdateZoneRecord", ZoneId=ZoneId, AccessKeyId=AccessKeyId)
...
if __name__ == '__main__':
    ...
    print("ALIYUN_ACCESS_KEY_ID:%s ALIYUN_ACCESS_KEY_SECRET:%s" %(ALIYUN_ACCESS_KEY_ID,ALIYUN_ACCESS_KEY_SECRET))
    ZoneId = sys.argv[1]
    main(ZoneId, ALIYUN_ACCESS_KEY_ID)
